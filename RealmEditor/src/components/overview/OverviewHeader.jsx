...
import { Search, Bell, Settings, LogOut, User, Code } from 'lucide-react';
import useSettingsStore from '../../store/useSettingsStore';
import InboxDrawer, { inboxItemsData } from './InboxDrawer'; // Import the drawer AND its data
...
export default function OverviewHeader({ onSettingsClick }) {
  ...
  const [hasUnread, setHasUnread] = useState(false); // State for the notification dot
  const profile = useSettingsStore((state) => state.profile);
  const accentColor = useSettingsStore((state) => state.accentColor);
  const navigate = useNavigate();

  // Check for unread items when the component loads
  useEffect(() => {
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-16 bg-card-background/80 backdrop-blur-lg border-b border-border-color">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            {/* Bell button with ringing animation and notification dot */}
            <motion.button 
