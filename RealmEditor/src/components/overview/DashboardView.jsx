...
import InboxPreviewPanel from './InboxPreviewPanel'; // Added import
import InboxDrawer from './InboxDrawer'; // Added import

// --- Consolidated Mock Data for all panels ---
const jumpBackInData = [
  // Restored to two items
...
// --- REMOVED inboxItemsData array ---

const quickActionsData = [
    { title: 'New Project', description: 'Start a workspace.', icon: Plus, color: 'bg-accent' },
    { title: 'New Task', description: 'Add a to-do item.', icon: CheckCircle, color: 'bg-accent' },
];
...
                    {isProject ? <GitCommit size={18} className="text-text-secondary flex-shrink-0" /> : <FileCode2 size={18} className="text-text-secondary flex-shrink-0" />}
...
    return (
        <motion.button
            className="relative group bg-card-background p-4 rounded-xl border border-border-color text-left w-full h-full shadow-sm overflow-hidden"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
...
                <motion.div 
                    className="p-2 rounded-lg bg-background/50 group-hover:bg-white/20"
                    variants={{ hover: { scale: 1.1 } }}
                    transition={{ type: 'spring', stiffness: 300, damping: 15 }}
                >
                    <Icon size={20} className="text-text-primary group-hover:text-white" />
                </motion.div>
                <div>
                    <p className="font-semibold text-text-primary group-hover:text-white">{action.title}</p>
                    <p className="text-sm text-text-secondary group-hover:text-white/80">{action.description}</p>
                </div>
            </div>
        </motion.button>
    );
...
            <div className="absolute left-8 top-2 h-full border-l-2 border-border-color"></div>
            
...
              whileHover={{ y: -3, backgroundColor: 'var(--background)' }}
            >
              <div 
...
export default function DashboardView() {
  const [isInboxDrawerOpen, setInboxDrawerOpen] = useState(false); // Added state
...
              </div>
            </div>
            {/* Added the Inbox Preview Panel here in its own section */}
            <div>
              <InboxPreviewPanel onOpenDrawer={() => setInboxDrawerOpen(true)} />
            </div>
          </div>
